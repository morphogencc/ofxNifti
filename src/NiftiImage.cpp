...
long NiftiImage::seek(long offset, int whence)
{
	if (_mode == NIFTI_CLOSED)	// Seek is private, so this should not happen
		std::printf("Cannot seek in a closed file.\n");
	...
	if (newpos == currpos)
		...
		std::printf("Attempted to seek into the header of file %s\n", _imgname.c_str());
	else if (_gz && (_mode == NIFTI_WRITE) && (newpos < currpos))
		std::printf("Cannot seek backwards while writing a file with libz.\n");
...
void NiftiImage::readHeader(string path)
{
	...
	if (!_file.zipped)
		std::printf("Failed to open header from %s\n", path.c_str());
	...
	if (obj_read < 1)
		std::printf("Could not read header structure from %s\n", _hdrname.c_str());
	...
	if (nhdr.sizeof_hdr != sizeof(nhdr)) {
		...
		if (nhdr.sizeof_hdr != sizeof(nhdr))
			std::printf("Could not determine byte order of header %s\n", _hdrname.c_str());
...
void NiftiImage::writeHeader(string path)
{
	...
	if(!_file.zipped)
		std::printf("Cannot open header file %s for writing.", _hdrname.c_str());
	...
	if(bytesWritten < sizeof(nhdr))
		std::printf("Could not write header to file %s\n", _hdrname.c_str());
	...
	{	// Close header and open image file
		...
		if (!_file.zipped)
			std::printf("Could not open image file %s for writing.\n", _imgname.c_str());
	}
}
...
char *NiftiImage::readBytes(size_t start, size_t length, char *buffer)
{
	...
	if (_mode == NIFTI_CLOSED)
	{
		std::printf("Cannot read from a closed file.\n");
		if (didAllocate) delete[] buffer;
		return NULL;
	}
	if (_mode == NIFTI_WRITE)
	{
		std::printf("Cannot read from a file opened for writing.\n");
		if (didAllocate) delete[] buffer;
		return NULL;
	}
	if (length == 0)
	{
		std::printf("Asked to read a buffer of 0 bytes.\n");
		if (didAllocate) delete[] buffer;
		return NULL;
	}
	...
	if (obj_read != 1)
		std::printf("Read buffer returned wrong number of bytes.\n");
...
void NiftiImage::writeBytes(char *buffer, size_t start, size_t length)
{
	if (_mode == NIFTI_CLOSED)
	{
		std::printf("Cannot write to a closed file.\n");
		return;
	}
	if (_mode == NIFTI_READ)
	{
		std::printf("Cannot write to a file opened for writing.\n");
		return;
	}
	seek(_voxoffset + start, SEEK_SET);
	size_t obj_written;
	...
	if (obj_written != 1)
		std::printf("Write buffer failed.\n");
}
...
bool NiftiImage::open(const string &filename, const char &mode)
{
	...
	if (_mode != NIFTI_CLOSED)
		std::printf("Attempted to open file %s using NiftiImage that is already open with file %s\n", filename.c_str(), _imgname.c_str());
	if (mode == NIFTI_READ) {
		...
		std::printf("Invalid NiftImage mode %d.\n", mode);
	}
	return true;
}
...
void NiftiImage::close()
{
	...
	// correct size.
	if (_mode == NIFTI_CLOSED) {
		std::printf("file %s is already closed.\n", _basename.c_str());
		return;
	}
	seek(0, SEEK_END);
...
void NiftiImage::setDatatype(const int dt)
{
	...
	if (it == DataTypes.end())
		std::printf("Attempted to set invalid datatype.\n");
	else
		_datatype = it->second;
}
